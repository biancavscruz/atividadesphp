...
<body>
    ...
    $desconto = 0.10;
    $imposto = 0.08;
    $valorfinal= number_format($valororiginal + ($valororiginal * $imposto), 2, ',', '.');
    $valorfinaldesconto = number_format($valororiginal - ($valororiginal * $desconto) + ($valororiginal * $imposto), 2, ',', '.');

    if ($valororiginal > 500) {
        echo "Parabéns você ganhou um desconto de 10%, sua compra antes custava: R$$valorfinal agora sairá por apenas: R$$valorfinaldesconto.";
    } else 
        echo "Sua compra deu um total de R$$valorfinal."; 
